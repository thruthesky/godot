...
실패로 잡힌다는 것. 그리고 🛑 **기물은 반드시 한 청크에 온전히 속한다**(ABSOLUTE) — 판정은
원점이 아니라 **AABB 전체**라 나무는 **잎사귀 끝까지** 들어와야 하고(경계를 넘은 나무 259개를
전수 조사하니 **원점은 100% 청크 안**이었다 — 원점만 검사하면 위반을 하나도 못 찾는다),
...
제거하지 못한다**(원점은 이미 100% 박스 안이다). 게다가 그 기물 폭은 도면 치수가 아니라
**회전이 키운 값**이다 — Y축 45° 에서 대각선이 그대로 폭이 되어 `7.68m` 기물이 `10.30m`,
스케일 ±10% 까지 곱하면 **11.33m(도면의 1.46배)** 가 된다(실측 역산 5.62m 와 이론 5.66m 가
**오차 4cm** 로 일치). 이어 **좁게 시작하는 법** — 크기가 아니라 `view_distance` 를
