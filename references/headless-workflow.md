...
🛑 **`.gdignore` 는 폴더 전용이고, 이름이 `.godotignore` 가 아니며, 내용을 적는 파일도 아니다.** 숨길 폴더 **안에 빈 파일**로 넣는다. 개별 파일을 독에서 치우는 법(확장자 화이트리스트)까지 포함한 전체 설명은 → [basics.md](basics.md) §6 "FileSystem 독에서 파일·폴더를 숨긴다"
