...
### `exclude_filter`로 번들 크기 줄이기 — 앱이 실행 중 읽지 않는 파일 빼기

기본 `export_filter="all_resources"`는 프로젝트 폴더 안의 리소스를 거의 다 번들에
담는다. 그런데 저장소에는 게임이 런타임에 `load()`하지 않는 파일도 섞여 있기
마련이다 — 테스트 씬(`tests/`), 예제(`examples/`), 제작 도구의 파이썬/셸 스크립트,
`__pycache__`, 미리보기용 몽타주 이미지, README에서만 참조하는 참고 자료 등.
이런 파일은 게임을 플레이하는 데 아무 쓸모가 없으면서 번들 용량만 차지한다.

`exclude_filter`에 글롭 패턴을 콤마로 나열하면 **내보낼 때만** 그 파일들을 뺀다.
디스크의 원본 파일을 지우는 것이 아니라 "이번 빌드에는 담지 마라"는 목록이라
언제든 되돌릴 수 있다.

```ini
exclude_filter="artifacts/*, builds/*, tests/*, examples/*, **/__pycache__/*, **/*_montage.png, **/*.verify.json"
```

**주의할 점**

- **플랫폼별로 다르게 준다** — 모바일 preset에는 자산 파일까지 빼는 경우가 있지만
  (예: 원격 다운로드팩으로 대체), 그 파일들을 여전히 앱 안에 담아야 하는 다른
  플랫폼 preset(데스크톱 등)에는 정크 파일만 빼고 자산은 빼지 않는다. 실행 시점에
  그 자산을 원격에서 받아오는 구조가 없는 플랫폼에서 자산까지 빼면 그냥 영영
  사라진다.
- **"참조가 없다"와 "안전하게 뺄 수 있다"는 다르다** — 코드가 파일명을 문자열
  조합(`"%s.png" % name`)으로 만들어 `load()`하면 정적 검색으로는 그 참조를 못
  찾는다. 사람이 이름·용도를 보고 게임 자산이 아님을 확인한 뒤에만 자산성 파일을
  필터에 추가한다. 반대로 도구·문서·테스트처럼 애초에 런타임 코드가 절대 읽을 수
  없는 종류의 파일은 비교적 안전하게 뺄 수 있다.
- **되돌리기** — 패턴을 지우면 다음 빌드부터 다시 포함된다. 원본은 애초에 안
  바뀌었으므로 별도 복구가 필요 없다.
