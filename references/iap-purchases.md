...
### 🛑 회수가 스스로 막히는 함정 — 팔지 않게 된 상품의 거래

**"거부되면 마무리하지 않는다" 는 규칙이 거꾸로 무는 자리가 있다.** 예전에 팔았지만 이제 지급할
것이 없는 상품(폐지한 구독·없어진 번들)의 거래가 기기에 남아 있으면:

```
회수가 제출 → 서버 "그런 상품 모른다" → 거부됐으니 마무리 안 함 → 그대로 남음 → 다음에 또 제출
```

**영원히 순환한다.** 실제 사례: iOS 샌드박스에 옛 구독 거래가 **84건** 쌓여, 상점을 열 때마다 서버를
84번 때리고 24초를 쓰고, **그 뒤에 줄 선 정상 구매의 회수까지 가렸다**(2026-09-12).

**해결 — 서버가 "버려도 된다" 를 명시한다.** 지급 카탈로그에 없으면 그 사실을 응답에 싣는다.

```jsonc
// 서버: 지급할 것이 아예 없는 상품 → 클라가 거래를 정리해도 된다
{ "ok": false, "reason": "unknown_product", "finish": true }
```

```gdscript
if not res.get("ok", false):
    # 🛑 서버가 확언한 거래만 마무리한다. 그 밖의 실패(검증 실패·통신 오류)에서 마무리하면
    #    돈을 낸 거래가 사라져 다시 제출할 길이 없어진다.
    if bool(res.get("finish", false)):
        await store.finish_transaction(purchase)   # 지급 없음 — 큐에서만 치운다
        return
    return _fail(...)                              # 남겨 둔다 → 다음 회수에서 재시도
```
...
| 규칙 | 왜 |
|---|---|
| **검증 전에** 판정한다 | 지급할 것이 없는 거래로 스토어 검증을 84번 때릴 이유가 없다 |
| `finish` 없는 응답은 **마무리하지 않는다** | 옛 서버와 붙어도 산 물건이 사라지지 않게 — 기본값이 안전한 쪽이어야 한다 |
| 회수는 **파는 상품을 먼저** · 한 번에 처리할 **상한** | 폐지 상품 뒤에 줄 서면 지급이 밀리고, 상한이 없으면 쌓인 수만큼 상점 열기가 늦어진다 |

### 🛑 판매 목록과 지급 카탈로그를 분리한다 — 그리고 **지급 쪽으로 판정한다**

상품을 판매 중단할 때 지급 카탈로그에서도 지우면 **과거에 산 사람의 지급이 사라진다.** 그래서 보통
목록을 둘로 나눈다. 그런데 **요청 검증을 판매 목록으로 하면 분리한 의미가 없어진다** — 판매 중단한
상품을 이미 산 사람의 회수가 **영구 거부**된다(실측 · 코드와 설계 문서가 정반대였다).

```
판매 목록(좁다)  → 무엇을 진열하나 · 새 구매를 받나
지급 카탈로그(넓다) → 무엇을 지급하나 · **회수 요청을 받나**  ← 요청 검증은 이쪽이다
```

### 🛑 거부 이유를 로그에 남긴다 — 영수증은 **길이만**

`요청 무효` 한 줄만 남기면 어느 필드가 어긋났는지 알 수 없어 **돈은 나갔는데 원인을 못 찾는** 상태가
된다. 필드 이름과 함께 남기고, `JSON.parse` 실패도 조용히 삼키지 말 것(payload 가 잘렸을 수 있다).

🛑 **영수증 본문은 절대 로그에 넣지 않는다.** 로그가 결제 자격 증명이 된다 — 길이만 남긴다.
...
- 🛑 **기기가 네트워크로만 붙어 있으면 `idevice_id -l` 이 비고 syslog 가 텅 빈다.** `idevice_id -n` 으로 UDID 를
  얻고 `idevicesyslog -n -u <UDID>` 로 받는다. 그대로 받으면 80초에 50MB 가 넘으니 걸러 받는다.
- 🛑 **스트림이 조용히 끊긴다.** 실측: 걸어 둔 지 16분 뒤 멈춰, 정작 구매 시각의 로그가 **0줄**이었다.
  같은 실행에서 앞부분은 잘 받았기 때문에 "로그가 없다 = 아무 일도 없었다" 로 오판하기 쉽다.
  → **진행이 멈추면 다시 붙이고**, 결정적 순간 전에 `wc -c` 로 크기가 늘고 있는지 확인할 것.
- 🛑 **`-o FILE`(파일 직접 쓰기)로는 로그가 흐르지 않았다**(실측). 동작하는 조합은 파이프뿐이다:
  `idevicesyslog -n -u <UDID> -p <프로세스> | grep --line-buffered -E '...' >> log`.
  `-m <문자열>`(메시지 필터)도 놓친 적이 있어, **`-p` 로 프로세스만 좁히고 내용은 grep** 이 안전하다.
- 🛑 **화면이 잠긴 기기에서는 `devicectl … process launch` 가 "Launched" 라고 해도 앱이 뜨지 않는다.**
  크래시 로그도 남지 않아 구별이 어렵다 — `devicectl device info processes` 에 없고 syslog 에 그 프로세스가
  0줄이면 **잠금부터 확인**할 것(빌드 문제로 몇 시간을 쓰기 전에).
...
### 🛑 구매 항목은 객체로도 사전으로도 온다

```gdscript
# 🛑 오류로 죽는다 — has_method() 는 Object 의 메서드다. Dictionary 에 부르면 회수 루프가
#    통째로 중단되어, 지급을 놓친 결제를 영영 되찾지 못한다(조용히 회수만 안 된다).
var d := p.to_dict() if p.has_method("to_dict") else p

# 타입을 먼저 가른다
func _to_purchase_dict(p) -> Dictionary:
	if p is Dictionary: return p
	if p is Object and p.has_method("to_dict"):
		var d = p.to_dict()
