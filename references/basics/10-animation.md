# 10. 애니메이션 — 캐릭터의 본과 기물의 셰이더

> **[Godot 기본](../basics.md)** 의 파트 **11 / 11**
> [← 9. 실전 — 3D 캐릭터 컨트롤러를 한 줄씩 읽는다](09-controller.md) · [← 색인으로 돌아가기](../basics.md)

> **이 문서로 오는 상황** — `.glb` 모델이 T-포즈로 서 있을 때 · 키 입력과 걷는 동작의 연결이 궁금할 때 · **본·AnimationPlayer 없는 분수가 움직이는 원리와 성능**을 알고 싶을 때
...
**10.0~10.10은 본 애니메이션을 가진 캐릭터 예제**다. 모든 움직임에 본이 필요한 것은
아니다. **10.11~10.12는 정적 GLB를 셰이더로 움직이는 분수**를 통해 다른 방식을 설명한다.
...
| [10.11](#1011-정적-glb도-움직인다--본-없는-셰이더-애니메이션) | 정적 GLB도 움직인다 — 본 없는 셰이더 애니메이션 |
| [10.12](#1012-성능에-유리한가--cpugpu메모리fps) | 성능에 유리한가 — CPU·GPU·메모리·FPS |
...
이 캐릭터 예제에서는 다음 세 가지를 구분한다.

| | 흔한 오해 | 실제 |
|---|---|---|
| ① | 이 캐릭터의 본 애니메이션은 **"움직이는 그림"** 이다 | **뼈의 시간별 자세를 적은 숫자표**다. 그림이 아니다 |
...
위 표는 **이 캐릭터 GLB의 구성**이다. GLB에 스켈레톤·스킨·애니메이션이 반드시 들어가는
것은 아니다. 정적 기물은 이 데이터가 없어도 정상이며, 노드 변환이나 모프 애니메이션도
가능하다. **GLB라는 확장자만으로 움직임의 유무나 방식을 판단하지 않는다.**
...
**본 애니메이션이 있어야 하는 캐릭터인데 목록이 비면**, 탐색한 노드·임포트 옵션과
10.1의 파일 목록을 대조한다. `ap`가 `null`이면 먼저 탐색 경로를 확인한다.
셰이더로 움직이는 기물은 `AnimationPlayer`가 없어도 정상이다(10.11).
...
**내장한 본 애니메이션을 재생하려는데 배열이 비어 있으면** 원본의 액션·내보내기 설정을
확인한다. 다른 파일의 애니메이션을 사용하거나 Godot에서 별도로 만드는 경우도 있으므로,
GLB의 빈 배열만으로 모든 애니메이션이 불가능하다고 판단하지 않는다.
...
| **분수 GLB만 넣었더니 물이 멈춰 있다** | 셰이더 머티리얼을 연결한 기물 씬 대신 원본 GLB를 배치했을 수 있다 | 10.11의 기물 `.tscn`과 머티리얼 연결 확인 |
...
## 10.11 정적 GLB도 움직인다 — 본 없는 셰이더 애니메이션

**파일 안의 정점 좌표가 고정되어 있어도, GPU가 그릴 때 좌표와 색을 바꾸면 화면에서는
움직인다.** Royal 모바일 분수는 이 방식이다. 본과 애니메이션 클립을 저장하는 대신
메시의 구역 표시와 작은 셰이더를 사용한다.

```text
정적 GLB → Godot용 메시 리소스
             돌 + 물줄기 + 물방울 + 수면을 surface 1개로 구성
                         ↓ 공유 ShaderMaterial
      UV2: 어느 부분인가       TIME: 현재 시각
                         ↓
      vertex(): 돌은 고정, 물줄기·물방울 정점은 이동
                         ↓
      fragment(): 수면의 낙수 지점에서 퍼지는 물결 무늬 계산
                         ↓
                화면에 움직이는 분수 1개
```
...
Royal 분수의 `UV2`는 두 번째 텍스처를 입히는 용도 대신 **구역과 흐름 진행도를 저장하는
데이터**로 쓴다. 이 표시는 제작 시 정점에 넣고 런타임에 그대로 읽는다.

| 부분 | 이 분수에서 하는 일 |
|---|---|
| 돌과 장식 | 원래 정점과 베이크한 색을 유지한다 |
| 물줄기·물방울 | 정점 셰이더가 시간과 궤적으로 위치·폭을 계산한다. 물방울마다 노드를 옮기지 않는다 |
| 수면 | 프래그먼트 셰이더가 낙수 위치·타이밍을 기준으로 퍼지고 약해지는 물결 무늬를 계산한다 |

수면은 **색의 변화로 물결이 보이는 방식**이며, 실제 유체 시뮬레이션이나 충돌 감지 결과가
아니다. 물방울 궤적과 물결이 같은 시간 규칙을 써서 연결되어 보인다. GPU에서 변형한
표시용 정점은 물리 콜리전을 자동으로 바꾸지 않는다.

Godot의 `TIME`을 사용하므로 이 분수에는 매 프레임 물 애니메이션을 갱신하는 GDScript나
`AnimationPlayer`가 필요 없다. 다만 **엔진의 씬 처리·컬링·렌더 명령 제출은 계속 필요**하다.
[공식 정점 애니메이션 예제](https://docs.godotengine.org/en/stable/tutorials/performance/vertex_animation/animating_thousands_of_fish.html)도
시간에 따라 GPU에서 정점을 움직이는 원리를 설명한다.

### 움직이는데 어떻게 1드로우콜인가

**드로우콜은 애니메이션 개수가 아니라 그리기 명령을 나누는 단위와 패스에 달려 있다.**
하나의 명령 안에서 각 정점이 서로 다르게 움직여도 된다.

이 분수는 돌과 물을 **메시 1개·surface 1개·머티리얼 1개·256×256 아틀라스 1장**으로
묶었다. 머티리얼은 불투명 `UNSHADED`이고 그림자와 추가 패스가 없다. 그 조건에서
**분수 1개의 실측 추가 드로우콜은 1**이었다. 화면 전체의 드로우콜이 1이라는 뜻은 아니다.

본이 있는 캐릭터도 단일 surface·머티리얼과 단일 패스 조건이면 1드로우콜이 가능하다.
**본을 없애서 자동으로 1드로우콜이 되는 것은 아니다.** 물을 별도 surface로 나누거나
추가 패스를 쓰면 늘어날 수 있고, 분수를 여러 개 배치해도 자동으로 전체가 1이 되지는 않는다.
반복물의 `MultiMesh` 묶음은 별도로 설계해야 한다.

### GLB만 배치하면 왜 멈추는가

이 모델의 움직임은 **Godot의 ShaderMaterial에 들어 있다.** 정적 GLB를 일반 뷰어에서
열거나 셰이더 연결 없이 배치하면 움직이지 않는다. 라리엔 3D에서는 다음 기물 씬을 배치한다.

`maps/props/fountains/fountain_royal_mobile/fountain_royal_mobile.tscn`

이 씬이 사용하는 메시와 `royal_unshaded.tres`, `royal_water.gdshader`,
`royal_flow_data.gdshaderinc`, 아틀라스가 함께 있어야 한다. 이 경우 확인할 것은
애니메이션 재생 버튼이 아니라 **올바른 기물 씬과 셰이더 머티리얼 연결**이다.

## 10.12 성능에 유리한가 — CPU·GPU·메모리·FPS

**분수처럼 반복되는 단순한 움직임에는 CPU와 애니메이션 데이터 비용을 줄이기 좋은
방식이다.** 다만 셰이더 계산은 GPU의 일이므로 FPS가 반드시 올라가지는 않는다.

### 어떤 비용이 달라지는가

| 항목 | 본·AnimationPlayer 방식 | 이 분수의 수식 기반 셰이더 방식 |
|---|---|---|
| CPU | 트랙 보간과 본 자세 갱신. 노드별 애니메이션이면 각 변환 갱신도 필요 | 이 애니메이션 작업을 생략. 일반 렌더링 CPU 비용은 남는다 |
| GPU | 보통 본 행렬과 가중치로 정점을 변형하는 스키닝 수행 | 정점 궤적 계산과 수면 픽셀의 물결 계산 수행 |
| 메모리·번들 | 본·스킨 가중치·클립 데이터 보관 | 이 데이터를 생략하고 구역 데이터·궤적 상수·셰이더 보관 |
| 드로우콜 | surface·머티리얼·패스·인스턴싱 조건에 따라 결정 | 같은 기준으로 결정. 셰이더 애니메이션 자체가 추가 드로우콜을 요구하지 않는다 |
| 표현 범위 | 복잡한 캐릭터 자세와 동작 혼합에 적합 | 분수·풀 흔들림처럼 수식으로 표현하기 쉬운 반복 운동에 적합 |

**본 애니메이션도 정점 변형은 보통 GPU가 한다.** 차이는 모든 일을 CPU에서 GPU로 옮기는
것이 아니라, CPU의 본 자세 계산을 없애고 GPU가 수식으로 직접 모양을 계산하는 데 있다.
이 분수는 프레임별 정점 위치를 큰 텍스처에 저장하는 **VAT가 아니므로 애니메이션 텍스처도
추가하지 않는다.** 반대로 복잡한 동작을 거대한 셰이더나 VAT로 바꾸면 메모리·GPU 비용이
더 커질 수 있다.

### 800삼각형·1드로우콜인데도 GPU가 느려질 수 있나

그렇다. **정점 계산량은 처리하는 정점 수에, 물결 계산량은 수면이 차지하는 픽셀 수에
영향을 받는다.** 삼각형 800개가 정점 800개라는 뜻도 아니다. 화면을 크게 덮는 수면,
많은 파동 계산, 겹치는 반투명 면은 삼각형과 드로우콜이 적어도 부담이 될 수 있다.
Royal의 불투명 단일 surface는 반투명 중첩 비용을 피하지만 수면의 픽셀 계산은 남는다.
[공식 GPU 최적화 문서](https://docs.godotengine.org/en/stable/tutorials/performance/gpu_optimization.html)는
드로우콜과 픽셀 처리 등 병목을 구분해 측정하도록 설명한다.

### FPS에는 얼마나 도움이 되는가

CPU가 본·노드 갱신 때문에 밀리는 장면이면 프레임 시간이 줄어들 여지가 있다.
이미 GPU가 느린 장면에서는 복잡한 물결 셰이더가 오히려 부담을 늘릴 수 있다.
또한 60fps 상한에 걸려 있으면 작업량이 줄어도 **화면의 FPS 숫자는 계속 60**일 수 있다.
그 사실만으로 배터리나 발열이 개선됐다고 단정하지 않는다.

Royal 분수는 **Galaxy A12, Android release, Compatibility 렌더러, 분수 1개를 표시한
검증 장면에서 평균 60.06fps·프레임 시간 p95 18.166ms**를 기록했다. p95는 측정한
프레임 시간의 95백분위 값이다. 이 기록은 **같은 품질의 본 버전과 비교한 A/B 결과가
아니므로 몇 % 더 빠르다는 근거로 쓰지 않는다.** 상세 리소스 수치와 측정 범위는
[저사양 실측 문서 §5.8](../lowend-3gb-60fps.md)에 있다.

비교할 때는 같은 실기기·release 빌드·카메라·해상도·표시 개수·프레임 상한을 맞추고,
워밍업 후 평균 FPS뿐 아니라 CPU/GPU 시간, p95, 메모리를 함께 본다. 시간값만 고정해
물을 멈추면 셰이더 계산은 계속 수행될 수 있다. **계산 비용을 비교하려면 해당 연산을
제거한 셰이더 변형과 비교해야 한다.** 분수 한 개의 통과 결과를 MMO 전체 부하의
60fps 보장으로 확대하지 않는다.
...
- [Animating thousands of fish](https://docs.godotengine.org/en/stable/tutorials/performance/vertex_animation/animating_thousands_of_fish.html) — 본 없이 정점 셰이더와 MultiMesh로 움직이기
- [GPU optimization](https://docs.godotengine.org/en/stable/tutorials/performance/gpu_optimization.html) — 드로우콜·정점·픽셀 비용과 병목 측정
- [Spatial shader reference](https://docs.godotengine.org/en/stable/tutorials/shaders/shader_reference/spatial_shader.html) — `TIME`·`VERTEX`·정점 및 프래그먼트 처리
