...
| 절 | 내용 |
|---|---|
| [·](#gdscript-문법의-기초--왕초보가-가장-먼저-막히는-것들) | GDScript 문법의 기초 — 왕초보가 가장 먼저 막히는 것들 |
| [·](#값을-바꾸는-것과-실제로-일어나는-것은-다르다) | 값을 바꾸는 것과 **실제로 일어나는 것**은 다르다 |
| [·](#생명주기--언제-불리는가) | 생명주기 — 언제 불리는가 |
| [·](#_ready-가-실행되지-않는다--파일이-있다고-실행되는-게-아니다) | `_ready()` 가 실행되지 않는다 — 파일이 있다고 실행되는 게 아니다 |
| [·](#pass-는-무엇인가--스크립트를-붙이면-처음-보게-되는-것) | `pass` 는 무엇인가 — 스크립트를 붙이면 처음 보게 되는 것 |
...
| `and` · `or` · `not` | 공식 스타일. **`&&`·`||`·`!` 도 동작하지만** 가독성 때문에 쓰지 않는다 ([gdscript.md §5](../gdscript.md)) |
