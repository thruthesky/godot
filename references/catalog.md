...
> 🗂 **이 문서는 색인이다(255줄 — 파트마다 100~120단어 요약 포함). 본문은 [`references/basics/`](basics/) 아래
> **11개 파트**에 있다.** 한 파일이 5,000줄에 가까워져 파트별로 나눴다.
>
> | # | 파트 | 다루는 것 | 줄 |
> |---|---|---|---|
> | 0 | [무엇부터 공부하나](basics/00-study-list.md) | 6단계 체크리스트 — 엔진 구조부터 캐릭터 애니메이션까지 | 91 |
...
> | 7 | [에디터 조작](basics/07-editor-input.md) | 궤도 회전과 프리룩·3버튼 없는 마우스·왼손 사용자 리바인딩 | 210 |
> | 8 | [동영상 강좌](basics/08-video.md) | 손으로 한 번 따라 만들어 보는 강좌 4개 | 147 |
> | 9 | [캐릭터 컨트롤러](basics/09-controller.md) | 3D 컨트롤러 전체 코드를 한 줄씩. 좌표 규약·중력·transform.basis | 1,097 |
...
### [mesh-geometry.md](mesh-geometry.md) — 메시의 구조 — 정점·모서리·삼각형·면

3D 형상이 무엇으로 이루어져 있는지를 계보 하나로 잇는다. 정점(vertex)이 위치가 아니라
노멀·UV·색을 함께 담은 **속성 묶음**이라는 정의에서 출발해, 모서리(edge)가 왜 저장되지
않는지, GPU 가 삼각형만 그리는 세 가지 이유, face 가 모델링(사각형 1장)과 렌더링(삼각형
1개)에서 다른 것을 가리키는 문제, 인덱스가 정점을 재사용하는 방식(같은 사각형이 정점
6개 → 4개), **기본 `BoxMesh` 의 꼭짓점 8개가 정점 24개가 되는 이유**(한 꼭짓점이 세 면에
속하고 노멀이 다르다), 서피스 1개 = 드로우콜 1개, `MeshInstance3D` 의 상속 사슬과 형상·배치
분리, 와이어프레임·오버드로우·언셰이디드로 눈으로 확인하는 법(에디터 메뉴와
...
### [dynamic-download.md](dynamic-download.md) — 동적 다운로드 (자산을 앱 밖에 두고 실행 중에 받는다)

**스토어 한도를 넘는 자산을 서버에 두고 실행 중에 PCK 로 받아 얹는 전 과정.**
[export-build.md §7](export-build.md) 의 패치 PCK 가 *이미 낸 앱을 고치는* 것이라면,
이 문서는 **처음부터 앱에 넣지 않은 자산**을 받는 것이다. 라리엔 3D 가 Android·iOS 로
팩 1개(974파일 · 181,973,692바이트)를 배포하며 겪은 실제 사고가 근거다.

| 절 | 내용 |
|---|---|
| §1 왜 나누나 | Play 200MB·안전선 160MB · **왕복 235~509ms 는 33KB 나 1.7MB 나 같다**(25파일 개별 11.81초 vs 묶음 1.95초) → 파일 단위로 쪼개지 않는다 · 🛑 **걸어서 닿는 곳은 팩 경계가 될 수 없다** |
| §2 PCK 마운트 | `load_resource_pack()` 이 `res://` 위에 겹쳐 **로더가 한 벌**로 끝난다 · 실측: 마운트 전 실패가 캐시에 남지 않아 **재시작 없이 교체된다** · 원시 PNG·JSON 을 올리면 로더가 두 벌이 되는 이유 |
| **§3 🛑 플랫폼 분기** ★ | **모바일만 받고 데스크톱(Steam)은 전부 번들**로 내는 구성. **런타임 게이트 × export 필터 2×2 표**와 한쪽만 바꿨을 때 나는 두 사고(데스크톱에서 자산이 영영 안 보임 / 받고도 번들 것을 쓰며 **오류 0줄**) · `OS.has_feature` 표 · `custom_features` 는 언제 필요한가 · 🛑 `exclude_filter` 는 빌드 때만 넣고 되돌린다 · 주소가 비면 저절로 꺼지는 안전판 · `project.godot` **절 이름** 사고 · 폴더 규칙(`remote/`) vs 파생 목록 |
| §4 무엇을 넣나 | 🛑 **리맵(`.import` 의 `[remap]`)이 빠지면 하나도 안 붙는다**(원본 `.glb` 를 빼자 팩의 84% 감소) · 다른 씬이 직접 참조하는 자산은 번들에 남긴다(청크가 통째로 사라진 사고) · 🛑 콜리전은 넣지 않는다 + 시각 슬롯 코드 |
| §5 서버 레이아웃 | catalog(no-cache) + 팩(immutable) · 🛑 **catalog 를 맨 마지막에** 발행 · `minClientVersion` 은 사람이 정한다 · egress 무료가 왜 결정적인가 · 세대는 쌓이므로 GC 가 필수 |
| §6 런타임 계약 10 | 세대 캐시 · 원자 rename · sha256 · 이어받기 · 스트림 기록 · idle 타임아웃 · **재시도는 catalog 부터** · 🛑 셀룰러 허용 · 🛑 **fps 를 게이트로 쓰지 않는다** · 로그를 "망/거부" 로 나눠 찍는 이유 |
| §7 Godot 고유 함정 | 🛑 **언마운트가 없다**(새 세대는 다음 부팅) · 🛑 `HTTPRequest` 가 Range 이어받기를 파괴한다 → `HTTPClient`+`seek_end()` 실측 코드 · 팩 용량은 임포트 설정이 정한다 · 큰 팩 해시가 한 프레임 1초 |
| §8~9 | 🛑 **에디터 검사는 이 기능을 검증하지 못한다** — 내보낸 앱으로 재는 9가지 · 체크리스트 |
...
### [asset-store-api.md](asset-store-api.md) — Asset Store API

`store.godotengine.org` 의 **공개 REST API**(사이트 어디에도 링크가 없어 찾기 어렵다)를 실제로
호출해 확인한 값으로 정리한다. **가장 중요한 결론은 "업로드 API 는 없다"** — 스펙 전량을 세어
확인한 결과 쓰기 계열은 `POST`/`DELETE /auth/token` 둘뿐이고 그것도 **API 키 자체를 만들고 지우는
것**이다. 애셋 생성·버전 등록·파일 업로드 경로는 존재하지 않으므로, 등록과 새 버전 푸시는 웹 UI
수동이고 자동화는 로드맵의 *Asset store CLI* 를 기다려야 한다. **API 키는 퍼블리싱용이 아니라
조회용**(레이트 리밋·비공개 데이터)이라는 점을 분명히 한다. 엔드포인트 14개 전량을 인증 요구
여부와 함께 표로 싣고 — **조회는 전부 인증이 필요 없다** — `/assets/`·`/releases/`·`/search/query/`
의 파라미터와 실제 응답을 담는다. 실무에서 걸리는 지점을 따로 모았다 — **페이지네이션이 두
방식**(`/assets/` 는 `page_size` + **응답 헤더 `x-pagination`**, 검색은 **`batch_size`** + 본문
`count` + `scroll` 커서)이고 목록 본문이 그냥 배열이라 헤더를 버리면 총 개수를 잃는다,
**`introspection` 은 키가 틀려도 200 이며 `authenticated` 가 문자열 `"False"`** 라 그대로 참으로
평가된다, **`ReleaseData.size` 의 스펙 타입이 `boolean` 으로 잘못 적혀 있다**(실제는 MB float),
`download_url` 은 **서명된 임시 URL** 이라 캐시하면 만료된다, **AI 사용 공개 정보는 등록 화면에만
있고 API 에는 없다**. 마지막으로 인증 없이 도는 **내 애셋 릴리스 감시 스크립트**(등록 전에는 404)와,
이 프로젝트의 `gohud` 를 **submodule 로 개발·소비하고 스토어에는 수동 등록한 뒤 이 API 로
검증·감시**하는 역할 분담을 정리한다. 확인 기준은 API `1.1.0`(2026-09-12 실측).
...
실패로 잡힌다는 것. 그리고 🛑 **기물은 한 청크에 속한다** — **원점(뿌리)이 청크 안인 것은 ABSOLUTE**,
모양도 청크 안이 원칙이지만 ✏️ **나무 잎사귀가 조금 넘는 것은 스트리밍에 해가 없어 경고로 둔다**(2026-09-17 보완 —
기물은 원점 청크와 함께 켜지고 꺼져 사라지는 지점이 시야 밖이고, 걷기 격자는 줄기 콜리전만 막는다.
실제로 문제가 되는 것은 **원점이 청크 밖 · 넘어간 칸에 청크 없음 · 수십 m 넘는 큰 기물** 셋뿐이다).
모양은 🛑 **돌린 AABB 가 아니라 실제 꼭짓점(볼록 껍질)으로 잰다** — 비스듬히 돈 상자는 모서리 빈 공간이
튀어나와 `plane_tree` 가 45° 에서 *상자 5.15m · 실제 잎사귀 3.51m* 로 1.64m 크게 잡혔다
(경계를 넘은 나무 259개는 **원점이 100% 청크 안**이었다),
반대로 **높이(Y)는 제한이 없다**(청크 좌표에 높이 축이 없어 `pos.y` 를 쓰지 않는다 — 옆으로만
언로드되므로 탑·빌딩은 안전하고 **옆으로 퍼지는 것만 위험**하다). 어기면 반쪽이 남는 것이
아니라 **가시 거리가 비대칭**이 되고(소속 청크에서 62m 벗어난 기물이 북 64m 에서 사라지는데
남 160m 에서는 보였다) 넘어간 칸에 청크가 없으면 **허공에 뜬 채 다가갈 수 없는 기물**이 된다.
**흩뿌리기가 이 규칙을 조용히 어긴다** — `ScatterShape` 박스는 *원점이 찍힐 범위*라
`박스 + 기물 폭` 이 청크를 넘고, 🛑 `Remove Outside` 모디파이어는 **원점만 보므로 한 개도
제거하지 못한다**(원점은 이미 100% 박스 안이다). 박스는 *기물의 실제 반경*만큼 줄이거나
청크 가장자리에 음수 모양 띠를 두른다 — ✏️ 회전이 폭을 키우는 것은 **속이 꽉 찬 네모 기물뿐**이고
둥근 나무는 어느 각도에서도 3.4~4.0m 그대로다(2026-09-17 정정 · 종전의 *"45° 에서 1.46배"* 는 돌린 상자의 값이었다). 이어 **좁게 시작하는 법** — 크기가 아니라 `view_distance` 를
...
### [walkable-grid.md](walkable-grid.md) — 통행 격자 ★ 서버가 보는 땅

**"격자는 씬을 찍은 사진이다"** 를 한 줄 요약으로 세운다. 서버에는 Godot 이 없으므로
씬의 걸을 수 있는 땅을 **32cm 칸마다 1비트**로 옮겨 두고(메인 1024m 맵 3,200×3,200 =
1,024만 칸 중 19.5% 가 걸을 수 있다), 서버는 그것으로 **이동 판정 · 스폰 위치 · 몬스터
길찾기** 세 곳을 판정한다. 클라는 스스로 벽을 판단하지 않고 서버의 `vel=0` SNAP 3회를
따라 선다. 만드는 법은 헤드리스 Godot 이 씬을 통째로 세워 칸마다 **수직 광선 → 기물
윗면 게이트(0.15m) → 캐릭터 캡슐 셰이프 질의** 를 하는 것이며, 파이썬 `.tscn` 파싱이
안 되는 이유(3단계 인스턴스 체인·`.glb` 콜리전)와 내비메시가 안 되는 이유(좁은 통로
32/32 → 0/32)를 실측으로 보인다. 캡슐 반경이 0.3 이 아니라 **칸 반대각선을 더한
0.526m** 라 약 1.05m 보다 좁은 통로가 막히는 것이 설계상 보수라는 점, 캡슐을 0.05m
띄우지 않으면 지면 자체에 닿아 빈 땅 10,000칸이 전부 막힌다는 점도 담는다.
본론은 🛑 **씬과 격자가 어긋날 때의 증상 표** — 화면에는 길인데 서버가 막음 · 화면에는
나무인데 서버가 열어 둠 · "테두리만 벽인 섬" 에 갇힘 · 스트리밍 중 바닥이 없어 떨어짐 ·
떠 있는 기물 밑 · 벽에 끊겨 몬스터에게 갈 수 없음 · 배포 누락 — 과 **판별 순서**
(서버 좌표 정지 → 세대 대조 S1~S4 · 칸 수가 아니라 비트 해시 → 재생성 → 씬),
배포 관문 G0~G4 와 **`ERROR:` 한 줄에도 멈추는 이유**(로드 실패한 기물이 격자에서
조용히 빠진다). 2026-09-15 실제 사례 셋(없는 데모 파일을 가리키는 스캐터 아이템 ·
벽 추가로 몬스터 140마리 도달 불가 · 청크 밖으로 나간 건물)과 **청크 스트리밍과 격자는
서로 모른다**(서버가 아직 안 붙은 청크 위에 PC 를 놓을 수 있다 · PC 가 선 청크를 떼면
안 된다), 실측 함정 10가지를 함께 둔다.
...
**임포스터(impostor)** 도 다룬다 — 미리 구워 둔 그림 한 장으로 먼 모델을 대신하는
**LOD 의 마지막 단계**이고 **스켈레톤 갱신이 사라지는 것이 가장 큰 이득**이라는 것,
카메라 정책에 따라 필요한 각도 수가 1→96 로 벌어져 텍스처가 0.5MB→45MB 가 된다는 표,
**직교 투영에서는 먼 것이 화면에서 작아지지 않아**(실측 — 10m 도 50m 도 494px 고정,
같은 조건 원근은 523→20px) **화면 크기 기반 자동 LOD 가 무력화되므로 임포스터가
사실상 유일한 수단이 된다**는 것, 그럼에도 **`visibility_range` 는 실제 거리로 판단해
직교에서도 정확히 작동한다**는 실측(24m 실메시 / 26m 임포스터)과 `visibility_range_fade_mode`
기본값이 `DISABLED` 라 켜지 않으면 툭 바뀌는 함정, 그리고 **AOI 액터 60 을
근거리 8 / 중거리 20 / 원거리 32 로 나눠 90 DC 에 맞추는 배분**과
**전환 거리 25m 가 A12 실측(애니 실메시 28마리가 한계)에서 나온 값**이라는 근거.
