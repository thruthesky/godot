...
> 🛑 라리엔 3D 의 맵 규칙(카메라 3축 고정 · 드로우콜 300 · 청크 **64m** · 상주 3×3 · 기물은 한 청크에 온전히)은
...
| [4](#4-청크-스트리밍) | 청크 스트리밍 · 🛑 [**기물은 한 청크에 온전히**](#-기물은-반드시-한-청크에-온전히-속한다-absolute) |
...
### 🛑🛑 기물은 반드시 **한 청크에 온전히** 속한다 (ABSOLUTE)

청크를 나눠 스트리밍하기로 한 순간, 맵 위의 모든 기물에 두 가지가 강제된다.

| | 규칙 |
|---|---|
| **① 소속** | 기물은 **어느 한 청크 씬의 자식**이어야 한다. 청크 밖(부모 맵 씬 직속)에 두면 스트리밍 대상이 아니라 **항상 켜져 있는 짐**이 된다 |
| **② 온전히** | 그 기물의 **AABB 전체**가 소속 청크 안에 들어가야 한다. 나무라면 줄기뿐 아니라 **잎사귀 끝까지**, 건물이라면 **처마·차양까지** |

#### 판정은 원점이 아니라 AABB 다

가장 흔한 오해가 *"뿌리(원점)만 청크 안에 있으면 된다"* 이다. **틀렸다.**

```gdscript
## 기물 AABB 는 보이는 것과 부딪히는 것을 모두 합친 것이다.
func prop_aabb(node: Node3D, base: Transform3D) -> AABB:
	var out := AABB()
	var found := false
	for mi in node.find_children("*", "VisualInstance3D", true, false):
		var b := (base.affine_inverse() * mi.global_transform) * (mi as VisualInstance3D).get_aabb()
		out = out.merge(b) if found else b
		found = true
	for cs in node.find_children("*", "CollisionShape3D", true, false):
		if (cs as CollisionShape3D).shape == null:
			continue
		var b := (base.affine_inverse() * cs.global_transform) * cs.shape.get_debug_mesh().get_aabb()
		out = out.merge(b) if found else b
		found = true
	return out
```

> **실측 (라리엔 3D · 2026-09-16)** — 경계를 넘은 나무 **259개를 전수 조사하니 원점은 100% 청크 안**이었다.
> 넘은 것은 잎사귀뿐이다. 원점만 검사했다면 **위반을 하나도 못 찾는다.**
...
| 축 | 규칙 | 이유 |
|---|---|---|
| **X · Z** | 🛑 잎사귀 끝까지 전부 들어와야 한다 | 옆 청크가 언로드될 때 어긋난다 |
| **Y** | ✅ **제한 없음** — 하늘까지 뻗어도 된다 | 위아래로는 나뉘지 않는다 |

> 그래서 **높은 것(탑·빌딩·키 큰 나무)은 안전하고, 옆으로 퍼지는 것(가지 넓은 나무·긴 건물·처마)만 위험하다.**

#### 어기면 무엇이 깨지는가

기물은 소속 청크의 자식이라 **통째로** 사라진다 — 반쪽은 남지 않는다. 대신 이것들이 깨진다.

| | 증상 |
|---|---|
| **가시 거리가 비대칭이 된다** | 로드·언로드 판정이 *기물 위치*가 아니라 **소속 청크 위치** 기준이다. 소속 청크에서 62m 벗어난 기물은 **북 64m 에서 사라지는데 남 160m 에서는 보였다**(실측) |
| **허공에 뜬다** | 넘어간 칸에 청크가 없으면 발밑에 지면이 없다. 걷기 격자를 지면에서 굽는다면 그 칸은 **영구히 못 걷는 칸**이 되어, 보이는데 다가갈 수 없는 기물이 된다 |
| **경계에서 어긋난다** | 청크 A 소속 기물이 청크 B 땅 위에 서 있으면, B 에 선 플레이어 눈앞에서 A 가 언로드될 때 기물만 사라진다 |

반대로 **청크 안에 있기만 하면 안전 거리가 보장된다.** 상주 반경 `r`, 청크 크기 `S`, 미리 로딩 `M` 일 때
기물이 어디에 놓이든 최소 `r·S + M` 만큼은 보인다 — 예: `r=1 · S=64 · M=20` 이면 **사방 84m**.

#### 흩뿌리기(ProtonScatter 등)가 이 규칙을 조용히 어긴다

스캐터의 `ScatterShape` 박스는 **원점이 찍힐 범위**이지 **기물이 차지할 범위가 아니다.**

```
실제 점유 폭 = 박스 크기 + 기물 AABB 폭
```

```
          청크 S                     박스 62.9 에 반폭 5.6m 나무를 뿌리면
┌──────────────────────────────┐     31.45 + 5.6 = 37.05  >  32   🛑
│ ┌──────────────────────────┐ │     원점은 박스 안(✅)인데 잎사귀가 넘는다
│ │ ●                      ● │ │
│ └──────────────────────────┘ │
└──────────────────────────────┘
```
...
막으려면 **박스를 기물 반폭만큼 줄이거나**, AABB 를 보는 커스텀 모디파이어를 쓴다.

```gdscript
안전한 박스 = 청크 크기 − (기물 AABB 최대폭)
```

#### 🛑 그 "기물 폭"은 도면 치수가 아니다 — 회전이 키운다

`randomize_transforms` 로 Y축 회전을 주면 **AABB(축 정렬 상자)가 커진다.** 45° 근처에서 대각선이 그대로 폭이 된다.

```gdscript
# w × d 짜리 기물을 Y축으로 deg 만큼 돌렸을 때의 AABB 폭
func rotated_extent(w: float, d: float, deg: float) -> Vector2:
	var t := deg_to_rad(deg)
	var c := absf(cos(t)); var s := absf(sin(t))
	return Vector2(w * c + d * s, w * s + d * c)
```

| 회전 | 7.68 × 6.88 m 기물의 AABB |
|---|---|
| 0° | 7.68 |
| **45°** | **10.30** ← 대각선 |
| 90° | 6.88 |

여기에 스케일 랜덤 ±10% 를 곱하면 **11.33m** — 도면(7.68m)의 **1.46배**다.
라리엔 3D 실측에서 역산한 최대 반폭 **5.62m** 는 이 이론값 5.66m 와 **오차 4cm** 로 일치했다.

> 🔑 **박스를 정할 때 쓸 폭은 `.glb` 도면 치수가 아니라 `회전 최대 AABB × 스케일 상한` 이다.**
> 이 47% 가 어디에도 적혀 있지 않아, 풀·꽃에 맞춰 튜닝한 박스를 큰 나무에 그대로 재사용하는 사고가 난다.
