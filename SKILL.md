...
### 🛑 검증 실행은 사람 화면에 창을 띄우지 않는다 — 헤드리스 · 가상 모니터

사람이 같은 컴퓨터에서 일한다. AI 가 스스로 돌리는 확인·검사는 **꼭 필요한 경우를 빼고 `--headless`** 로 돌린다.
화면을 봐야 하는 검증은 **가상 모니터**에서 한다 — ① `scripts/xvfb_run.sh` 로 실행 → ② 검사 스크립트가 스크린샷 저장 → ③ 판정 숫자와 **PNG 를 직접 열어** 검증.
🛑 `--headless` 로는 스크린샷이 **어떤 우회로로도** 나오지 않는다(엔진 소스: 헤드리스 렌더링 드라이버는 `dummy` 하나). macOS 의 Godot 창은 화면 밖 좌표·최소화·`no_focus` 로도 숨겨지지 않는다(실측).
창을 띄웠다면 그 이유를 보고에 적는다.
가상 모니터 절차·촬영 검사 뼈대·함정 → [references/virtual-monitor.md](references/virtual-monitor.md) · 헤드리스에서 되는 것·안 되는 것 → [references/headless-workflow.md §2-A](references/headless-workflow.md)
...
| 폴리곤은 아끼지 않아도 된다 | 지형 삼각형 20만까지 실질 무해 · 마노라 섬 기물 313k→186k(−40%)를 한 실행 안에서 번갈아 재도 fps 차가 재현 오차 안(35.2↔35.1, 2026-09-10) — 같은 실행의 발열 스로틀(2.30→1.88GHz)이 35→26fps 로 더 컸다 |
...
| 실제로 어떻게 그려지나 (스크린샷·녹화) | **가상 모니터** `scripts/xvfb_run.sh` — 사람 화면에 창을 띄우지 않고 실행 → 캡처 → PNG 를 열어 검증 ([virtual-monitor.md](references/virtual-monitor.md)) |
| 특정 시점의 지역 변수 | DAP 브레이크포인트 |
| 성능 병목 · fps 하락 | 🛑 **[references/perf-tuning-playbook.md](references/perf-tuning-playbook.md)** — 진단 5단계를 순서대로 |
| 최적화를 **어떤 사고로** 하나 · 프로파일러 화면 읽기 | [references/rendering-cpu-optimization.md](references/rendering-cpu-optimization.md) — 엔진 팀의 5단계·사례 2건(고도 공식 블로그 정리) |
...
| [rendering-cpu-optimization.md](references/rendering-cpu-optimization.md) | 렌더러 **CPU** 최적화를 엔진 팀이 하는 법 — 5단계 방법론·프로파일러 고르기·화면 읽기·사례 2건 | 최적화 절차가 막막할 때 · 매 프레임 재생성이 의심될 때 |
...
| [protonscatter.md](references/protonscatter.md) | ProtonScatter 애드온 — 자유 배치 + 콜리전을 얻는 10단계 · **걸러내기**(모디파이어 스택 원리 · Remove Outside 는 원점만 · 라리엔 로컬 `Remove Outside (Model)` · `Footprint Multiplier` 0 금지 · `Keep Partial Overlap` · `Amount` 로 밀도 되살리기 · 꺼 둔 모디파이어가 게임에서 도는 함정 · 0개면 `build_completed` 없음) | 나무·바위를 흩뿌리며 충돌도 필요할 때 · 흩뿌린 기물을 청크 경계 안에 가두거나 나무가 너무 적어졌을 때 |
...
| [headless-workflow.md](references/headless-workflow.md) | 에디터 없이 작업하기 ★ 기본 작업 방식 · 🛑 `--headless` 로 되는 것·안 되는 것(스크린샷 null·입력 좌표·종료 조건) · 화면 촬영은 아래 가상 모니터 | [상세](references/catalog.md#headless-workflowmd--에디터-없이-작업하기--기본-작업-방식) |
| [virtual-monitor.md](references/virtual-monitor.md) | 🖥 **가상 모니터** — 화면에 안 보이게 실행해 스크린샷·녹화를 찍고 검증한다 · 실행 → 캡처 → 검증 3단계 · 복사해 쓰는 촬영 검사 뼈대 · `xvfb_run.sh` · 증명·속도·함정 | AI 가 사람 화면 없이 화면을 검증할 때 · "헤드리스로 스크린샷이 되나?" |
...
실행 → 캡처 → 검증 절차·촬영 검사 뼈대·실측·함정(`--init`·녹화 크기·GDExtension 오류) → [references/virtual-monitor.md](references/virtual-monitor.md)
