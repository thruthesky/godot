...
> 🛑 라리엔 3D 의 맵 규칙(카메라 3축 고정 · 드로우콜 300 · 청크 **64m** · 상주 3×3 · 기물은 한 청크에 — 원점은 반드시 · 나무 잎사귀 2m 이내는 허용)은
...
| [4](#4-청크-스트리밍) | 청크 스트리밍 · 🛑 [**기물은 한 청크에 — 원점 ABSOLUTE · 잎사귀 조금은 허용**](#-기물은-한-청크에-속한다--원점은-absolute--잎사귀가-조금-넘는-것은-괜찮다-2026-09-17-보완) |
...
### 🛑🛑 기물은 한 청크에 속한다 — 원점은 ABSOLUTE · 잎사귀가 조금 넘는 것은 괜찮다 (2026-09-17 보완)

청크를 나눠 스트리밍하기로 한 순간, 맵 위의 모든 기물에 규칙이 생긴다.

| | 규칙 | 강도 |
|---|---|---|
| **① 소속** | 기물은 **어느 한 청크 씬의 자식**이어야 한다. 청크 밖(부모 맵 씬 직속)에 두면 스트리밍 대상이 아니라 **항상 켜져 있는 짐**이 된다 | 🛑 **ABSOLUTE** |
| **② 원점(뿌리)** | 기물의 **원점이 소속 청크 안**에 있어야 한다 | 🛑 **ABSOLUTE** |
| **③ 모양** | 기물의 **실제 모양(메시 꼭짓점)** 도 소속 청크 안에 들어가는 것이 원칙이다. 🔑 단 **나무 잎사귀처럼 옆으로 조금 퍼진 부분이 옆 칸으로 넘는 것은 스트리밍에 영향이 없다** — 오류가 아니라 **경고**로 둔다 | ⚠ 원칙 · 조금은 허용 |

> ✏️ **2026-09-17 정정** — 이 절은 처음에 *"AABB 전체가 들어가야 한다 · 나무라면 잎사귀 끝까지 · ABSOLUTE"* 로 적었다.
> 라리엔 3D 실측으로 두 가지를 바로잡았다. **① 잎사귀가 1~2m 넘는 것은 스트리밍에 해가 없다**(아래 ❓).
> **② 돌린 AABB 는 실제 모양보다 크게 잰다** — 비스듬히 돈 나무는 상자 모서리(빈 공간)가 잎사귀보다 최대 1.68m 더 나와,
> 눈으로는 청크 안인데 검사가 실패했다(아래 "판정은 실제 모양으로").

#### ❓ 청크 스트리밍을 위해 잎사귀가 반드시 청크 안에 있어야 하나? — **아니다** (2026-09-17)

**잎사귀가 조금 넘는 것은 스트리밍에 영향이 없다.**

- 기물은 **원점(뿌리)이 있는 청크의 자식**이라, 그 청크와 **함께 켜지고 꺼진다.** 반쪽만 남지 않는다.
- 게임은 플레이어가 선 청크와 **주변 8칸**을 켜 둔다(상주 3×3). 그래서 원점이 청크 안에 있는 기물은
  플레이어가 **적어도 `r·S + M − (청크 반폭)`** 멀어져야 꺼진다 — 라리엔 3D(`r=1 · S=64 · M=20`)는 **52m**.
- 화면에 보이는 거리는 약 **30m** 라서, 잎사귀가 1m 넘은 나무가 꺼지는 순간은 **화면에 보이지 않는다.**
- **걷기 격자에도 영향이 없다.** 격자를 콜리전으로 굽는다면 나무가 길을 막는 부분은 **줄기**(라리엔 3D `plane_tree` 반지름 0.25m)뿐이다.

**스트리밍에서 실제로 문제가 되는 경우는 따로 있다.**

| 경우 | 증상 |
|---|---|
| 🛑 **원점(뿌리)이 청크 밖** | 로드·언로드가 *기물 위치*가 아니라 **소속 청크 위치** 기준이라 가시 거리가 비대칭이 된다. 소속 청크에서 62m 벗어난 기물은 **북 64m 에서 사라지는데 남 160m 에서는 보였다**(라리엔 3D 자유의 여신상 실측) |
| 🛑 **넘어간 칸에 청크가 없다** | 발밑에 지면이 없어 **허공에 걸친다.** 걷기 격자를 지면에서 굽는다면 그 칸은 **영구히 못 걷는 칸**이 된다 |
| 🛑 **기물이 청크를 수십 m 넘을 만큼 크다** | 원점이 안에 있어도 넘은 부분이 상주 범위 밖까지 뻗어, 옆 칸에 선 플레이어 눈앞에서 통째로 사라진다 |

> 🔑 그래서 **흩뿌린 나무 잎사귀가 1~2m 넘는 것은 게임에서는 무시해도 된다.**
> 라리엔 3D 는 **나무 잎사귀가 2m 이내로 넘고, 원점이 청크 안이며, 넘어간 칸에 청크가 있으면 ⚠ 노란 경고**로 두고
> 그 밖은 🛑 로 잡는다(2026-09-17 사람 결정 · `tests/verify_chunk_props.gd` `TREE_LEAF_TOLERANCE_M`).
> 허용 폭은 게임마다 **상주 반경·미리 로딩·시야**로 정한다 — 위 식의 보장 거리가 시야보다 넉넉히 길 때만 허용한다.

반대로 **원점이 청크 안에 있기만 하면 안전 거리가 보장된다.** 상주 반경 `r`, 청크 크기 `S`, 미리 로딩 `M` 일 때
기물이 어디에 놓이든 최소 `r·S + M` 에서 *원점이 청크 중심에서 벗어난 거리*를 뺀 만큼은 보인다 — 예: `r=1 · S=64 · M=20` 이면 중심 **84m** · 가장자리 **52m**.

#### 판정은 원점만이 아니라 **실제 모양**으로 — 🛑 돌린 AABB 로 재지 않는다

*"원점만 청크 안이면 된다"* 는 반만 맞다 — 기물이 크면(위 표 세 번째) 원점만으로는 못 잡는다. 그래서 모양도 잰다.
🛑 **그런데 모양을 AABB(축 정렬 상자)로 재고 그 상자를 기물과 함께 돌리면 틀린다.** 상자의 **모서리는 빈 공간**인데,
비스듬히 돌면 그 모서리가 X·Z 축 쪽을 향해 실제 모양보다 튀어나온다.

라리엔 3D `plane_tree`(꼭짓점 9,345개) 를 Y축으로 돌리며 원점~끝 거리를 쟀다(2026-09-17 실측):

| 회전 | 돌린 상자 | **실제 잎사귀** | 차이 |
|---|---|---|---|
| 0° | 3.84m | **3.84m** | 0 |
| 15° | 4.58m | **3.97m** | 0.61m |
| 30° | 5.01m | **3.87m** | 1.14m |
| **45°** | **5.15m** | **3.51m** | **1.64m** |
| 60° | 5.05m | **3.37m** | **1.68m** |
| 90° | 3.84m | **3.84m** | 0 |

**실제 잎사귀는 어느 각도에서도 3.4~4.0m 인데, 돌린 상자만 1.3배로 커진다.** 종전 검사는 이 상자로 재서
눈으로는 청크 안인 나무를 🛑 로 잡았다 — 같은 청크(`chunk_04_10`) 나무 하나가 *상자 +0.94m 넘음 · 실제 잎사귀 −0.50m 안쪽*이었다.

> 🔑 **회전이 폭을 키우는 것은 속이 꽉 찬 네모 기물뿐이다.** 벤치·컨테이너·긴 건물은 실제로 대각선만큼 넓어지고,
> 둥근 나무·바위는 거의 그대로다. 어느 쪽이든 **실제 꼭짓점으로 재면 저절로 맞다** — 기물 모양을 따로 가정하지 않는다.

**꼭짓점 전부를 매번 옮기면 느리므로, 메시마다 볼록 껍질(convex hull)의 꼭짓점만 한 번 구해 둔다.**
회전·기울기·크기를 어떻게 줘도 *꼭짓점 전부를 옮긴 범위*와 **같은 값**이 나온다(선형 변환의 최댓값은 볼록 껍질 꼭짓점에서 나온다).

```gdscript
var _hull_cache := {}

## 메시 하나의 볼록 껍질 꼭짓점(메시 로컬). 메시 리소스마다 한 번만 구한다.
func mesh_hull_points(mesh: Mesh) -> PackedVector3Array:
	var key := mesh.get_instance_id()
	if _hull_cache.has(key):
		return _hull_cache[key]
	var pts := PackedVector3Array()
	var box := mesh.get_aabb()
	if minf(box.size.x, minf(box.size.y, box.size.z)) < 0.001:
		# 🛑 평면(두께 0) 메시는 볼록 껍질 계산이 실패해 ERROR 를 찍는다 — 꼭짓점을 그대로 쓴다.
		for si in mesh.get_surface_count():
			pts.append_array(mesh.surface_get_arrays(si)[Mesh.ARRAY_VERTEX])
	else:
		var shape := mesh.create_convex_shape(true, false)   # clean=true · simplify=false → 정확한 껍질
		if shape != null:
			pts = shape.points
	if pts.is_empty():
		for i in 8:
			pts.append(box.get_endpoint(i))                    # 못 읽으면 상자 모서리 — 크게 잴 뿐 작게 재지 않는다
	_hull_cache[key] = pts
	return pts

## 기물의 실제 모양이 차지하는 범위(base 기준). 돌린 AABB 가 아니다.
func prop_extent(node: Node3D, base: Transform3D) -> AABB:
	var out := AABB()
	var found := false
	for mi: MeshInstance3D in node.find_children("*", "MeshInstance3D", true, false):
		if mi.mesh == null:
			continue
		var t := base.affine_inverse() * mi.global_transform
		for p in mesh_hull_points(mi.mesh):
			var w := t * p
			out = out.expand(w) if found else AABB(w, Vector3.ZERO)
			found = true
	return out
```

> 메시가 없고 콜리전만 있는 기물은 `CollisionShape3D` 모양의 AABB 로 잰다(상자 콜리전이면 모서리가 곧 실제 모양이다).
> **실측 (라리엔 3D · 2026-09-16)** — 경계를 넘은 나무 259개는 원점이 **100% 청크 안**이었다. 넘은 것은 잎사귀뿐이다.

#### 🔑 수평(X·Z)만이다 — 높이(Y)는 제한이 없다

청크 좌표에는 **높이 축이 없다.**

```gdscript
static func world_to_chunk(pos: Vector3) -> Vector2i:
	return Vector2i(floori((pos.x + half) / CHUNK_SIZE_M),
	                floori((pos.z + half) / CHUNK_SIZE_M))   # pos.y 는 쓰지 않는다
```

청크는 **바닥에 그은 격자**이고 그 위로는 무한 기둥이다. 언로드는 옆으로만 일어나므로
*"위쪽 청크"* 라는 것이 없고, 따라서 꼭대기가 잘릴 일도 없다.

| 축 | 규칙 | 이유 |
|---|---|---|
| **X · Z** | 원점은 반드시 안 · 모양은 원칙적으로 안(잎사귀 조금은 경고) | 옆 청크로 넘어간다 |
| **Y** | ✅ **제한 없음** — 하늘까지 뻗어도 된다 | 위아래로는 나뉘지 않는다 |

> 그래서 **높은 것(탑·빌딩·키 큰 나무)은 안전하고, 옆으로 크게 퍼지는 것(긴 건물·넓은 처마)만 위험하다.**

#### 흩뿌리기(ProtonScatter 등) — 박스는 원점이 찍힐 범위다

스캐터의 `ScatterShape` 박스는 **원점이 찍힐 범위**이지 **기물이 차지할 범위가 아니다.**

```
실제 점유 = 박스 반폭 + 기물의 실제 반경(원점~모양 끝)
```

```
          청크 S = 64                박스 60(반폭 30)에 실제 반경 4.4m 나무를 뿌리면
┌──────────────────────────────┐     30 + 4.4 = 34.4  >  32   → 가장자리 나무의 잎사귀가 2.4m 넘는다
│ ┌──────────────────────────┐ │     원점은 박스 안(✅)이다
│ │ ●                      ● │ │
│ └──────────────────────────┘ │
└──────────────────────────────┘
...
🛑 **`Remove Outside` 모디파이어로는 못 막는다.** 그 모디파이어는 `transforms.list[i].origin`,
즉 **원점만** 본다. 원점은 이미 100% 박스 안이므로 **한 개도 제거되지 않는다.**
막으려면 **박스를 기물 반경만큼 줄이거나**, 청크 가장자리에 **음수(`negative`) 모양**으로 띠를 두른다.

```gdscript
안전한 박스 = 청크 크기 − 2 × (기물의 실제 반경 × 스케일 상한)
```

> 기물의 실제 반경은 위 `prop_extent` 로 한 번 재 둔다 — **도면 치수도, 돌린 상자도 아니다.**
> 잎사귀 조금은 허용하는 게임이라면(위 ❓) 그만큼 박스를 덜 줄여도 된다.

#### 검증 — 눈으로 넘기지 않는다

스트리밍은 잘 돌 때 화면에 아무 일도 안 일어나므로(§9), **기물 경계는 반드시 숫자로 검사한다.**

```gdscript
const LEAF_TOLERANCE_M := 2.0   # 잎사귀 허용 폭 — 게임마다 정한다(위 ❓)

var half := CHUNK_SIZE_M * 0.5
var box := prop_extent(prop, chunk.global_transform)
var origin := chunk.global_transform.affine_inverse() * prop.global_position
if absf(origin.x) > half or absf(origin.z) > half:
	push_error("%s 원점이 청크 밖이다" % prop.name)              # 🛑 늘 오류
for d in [["동(+X)", box.end.x - half], ["서(−X)", -half - box.position.x],
          ["남(+Z)", box.end.z - half], ["북(−Z)", -half - box.position.z]]:
	if d[1] <= 0.0:
		continue
	if is_leafy(prop) and d[1] <= LEAF_TOLERANCE_M and neighbor_chunk_exists(d[0]):
		push_warning("%s 잎사귀가 %s 로 %.2fm 넘었다(허용)" % [prop.name, d[0], d[1]])   # ⚠ 경고
	else:
		push_error("%s 가 %s 로 %.2fm 넘었다" % [prop.name, d[0], d[1]])                 # 🛑 오류
```

방향이 **넷뿐인 것이 정상**이다 — 위·아래는 검사하지 않는다.
흩뿌린 기물은 `ProtonScatter` 가 만든 변환 목록을 순회해 **개체 하나하나**를 같은 식으로 잰다.
...
> 🛑 **청크를 쓴다면 §4 의 기물 규칙이 흩뿌리기에도 그대로 적용된다.**
> 스캐터 박스는 *원점이 찍힐 범위*라 **박스 + 기물 폭**이 청크를 넘기 쉽고,
> 그 기물 폭은 회전 때문에 도면 치수보다 최대 **1.5배** 크다 →
> [§4 흩뿌리기가 이 규칙을 조용히 어긴다](#흩뿌리기protonscatter-등가-이-규칙을-조용히-어긴다)
