...
| 절 | 내용 |
|---|---|
| [9.1](#91-먼저-씬-구조를-본다--코드보다-이게-먼저다) | 먼저 씬 구조를 본다 — 코드보다 이게 먼저다 |
| [9.2](#92-extends-characterbody3d--어떤-몸을-고를-것인가) | `extends CharacterBody3D` — 어떤 "몸"을 고를 것인가 |
| [9.3](#93-godot-의-3d-좌표-규약--왕초보가-가장-먼저-넘어지는-곳) | Godot 의 3D 좌표 규약 — 왕초보가 가장 먼저 넘어지는 곳 |
| [9.4](#94-전체-코드) | 전체 코드 |
| [9.5](#95-export--코드를-고치지-않고-값을-바꾼다) | `@export` — 코드를 고치지 않고 값을 바꾼다 |
| [9.6](#96-onready-와---자식-노드를-붙잡는-두-도구) | `@onready` 와 `$` — 자식 노드를 붙잡는 두 도구 |
| [9.7](#97-입력-함수-4형제--어느-것을-언제-쓰나) | 입력 함수 4형제 — 어느 것을 언제 쓰나 |
| [9.8](#98-시점-회전--왜-몸통과-카메라를-나눠서-돌리나) | 시점 회전 — 왜 몸통과 카메라를 나눠서 돌리나 |
| [9.9](#99-_physics_process--이동은-반드시-여기에-쓴다) | `_physics_process()` — 이동은 반드시 여기에 쓴다 |
| [9.10](#910-중력과-점프) | 중력과 점프 |
| [9.11](#911-이동-입력--inputget_vector-와-inputmap) | 이동 입력 — `Input.get_vector()` 와 InputMap |
| [9.12](#912-transformbasis--이-스크립트에서-가장-어려운-한-줄) | `transform.basis` — 이 스크립트에서 가장 어려운 한 줄 |
| [9.13](#913-속도를-정하고-실제로-움직인다) | 속도를 정하고 실제로 움직인다 |
| [9.14](#914-마우스-캡처--편리하지만-위험한-한-줄) | 마우스 캡처 — 편리하지만 위험한 한 줄 |
| [9.15](#915-이-예제가-3인칭인-이유와-1인칭으로-바꾸는-법) | 이 예제가 3인칭인 이유와 1인칭으로 바꾸는 법 |
| [9.16](#916-자주-넘어지는-곳--증상으로-찾는-표) | 자주 넘어지는 곳 — 증상으로 찾는 표 |
| [9.17](#917-직접-해-볼-것) | 직접 해 볼 것 |
...
[§1 의 "3D 게임에서 실제로 만나는 노드들"](01-world.md#3d-게임에서-실제로-만나는-노드들--몸--모양--그림)
...
코드에서 아예 도 단위로 다루고 싶으면 **`rotation_degrees`** 프로퍼티가 있다 — 인스펙터가 보여 주는 바로 그 값이다.
...
`_unhandled_input()` 이라는 낯선 이름이 나왔다. Godot 의 입력 콜백은 다섯이고(여기서는 넷만 쓴다)
**들어온 입력이 이 순서대로 흘러간다** (공식 *Using InputEvent* · [input-ui.md §1](../input-ui.md) 의 5단계와 같다).
...
② Control 노드의 UI 처리   ← 버튼·텍스트 입력창 등이 여기서 입력을 "먹는다"
   │
   ▼
③ _shortcut_input()      ← 단축키(키·조합키)만. 이 코드는 안 쓴다
   │
   ▼
④ _unhandled_key_input() ← UI 가 먹지 않은 키보드 입력만
   │
   ▼
⑤ _unhandled_input()     ← UI 가 먹지 않고 남은 모든 입력  ★ 이 코드가 쓰는 것

(🛑 예전 판은 ④와 ⑤ 의 순서가 뒤바뀌어 있었다 — 2026-09-03 공식 문서 대조로 정정)
```
