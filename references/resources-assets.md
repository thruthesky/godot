...
### `size_limit` — 원본은 그대로 두고 임포트 결과만 줄이기 (모바일·저사양에 유용)

모바일·저사양 타깃에서 "텍스처 해상도를 낮춘다"고 하면 원본 이미지를 다시 그리거나
포토샵으로 리사이즈해 덮어써야 한다고 생각하기 쉽지만, Godot에서는 **원본 파일을 한 바이트도
바꾸지 않고** 임포트 결과(GPU가 실제로 읽는 `.ctex`)만 줄일 수 있다.

#### 왜 모바일은 원본 해상도가 굳이 필요 없는가

- **화면이 그만큼 안 된다** — 저사양 Android 기기는 보통 720×1600~1080×2340급이다.
  텍스처가 화면의 일부만 덮는다면, 그 텍스처가 실제로 맵핑되는 화면 픽셀 수보다
  높은 텍셀 밀도는 버려지는 정보다.
- **카메라 거리·화각이 텍셀 대 픽셀 비율을 결정한다** — 오브젝트가 화면에서 차지하는
  픽셀 수가 애초에 수백 픽셀 안팎이면, 그 이상의 텍스처 해상도는 밉맵으로 다시
  뭉개진다(아래 항목). 고정 카메라·직교 투영처럼 오브젝트가 항상 작게 보이는
  구도라면 이 여유가 더 크다.
- **밉맵이 이미 원거리에서 다운샘플한다** — `Mipmaps/Generate`를 켜면(3D는 필수)
  GPU는 거리에 따라 자동으로 더 작은 레벨을 골라 읽는다. 원본 레벨(가장 큰 밉)은
  카메라가 아주 가까이 붙었을 때만 쓰인다.
- **저사양 기기의 실제 병목은 화질이 아니라 VRAM·대역폭이다** — 압축 포맷(ETC2/ASTC)
  크기는 가로×세로에 비례해 커지므로, 한 변을 2048→1024로 반 줄이면 용량은 1/4로,
  1024→512면 1/16로 준다. 3GB급 기기는 이 여유가 프레임 드롭·앱 크래시로 바로
  드러난다.

#### UV와는 무관하다 — 배치·크기를 안 건드린다

메시의 UV 좌표는 텍스처를 0~1 비율로 가리키는 정규화 좌표라서, 텍스처 픽셀 크기가
2048이든 1024든 **같은 지점을 가리킨다.** 즉 `size_limit`을 바꿔도 기물의 화면 속
위치·크기·UV 배치는 전혀 바뀌지 않는다. 안전하게 되돌릴 수 있는 순수한 화질 손실
조정이라는 뜻이다.
...
1. FileSystem 도크에서 텍스처 선택 → **Import** 도크 → **Advanced Import Settings**
2. **Process** 탭 → **Size Limit** 필드에 `1024`(또는 원하는 상한)를 입력
3. 다시 임포트(Reimport) — 긴 변이 이 값을 넘으면 그 값까지만 줄어든다.
   원본보다 작은 텍스처를 억지로 키우지는 않는다(0 = 무제한).

이 값은 소스 옆의 **`<파일명>.import`** 텍스트 파일에 한 줄로 저장된다.
...
| 파일 | 바뀌는가 | 비고 |
|------|---------|------|
| 원본 이미지 (`.png`/`.jpg`/…) | **바뀌지 않는다** | 바이트 단위로 그대로 |
| `<파일명>.import` | **바뀐다** | `process/size_limit=1024` 한 줄 추가/수정 — 텍스트라서 diff로 리뷰 가능 |
| `.godot/imported/*.ctex` (임포트 캐시) | **재생성된다** | GPU가 실제로 읽는 산출물. 지워도 재임포트하면 다시 만들어진다 |

되돌리기도 그 한 줄을 지우거나 `0`으로 바꾸고 재임포트하면 끝이다.

#### 여러 파일에 한 번에 적용 — 헤드리스 재임포트

파일 수가 많으면 에디터에서 하나씩 여는 대신, `.import` 파일을 텍스트로 직접
고치는 쪽이 빠르다. 기존에 `process/size_limit=` 줄이 있으면 값을 바꾸고,
없으면 `[params]` 섹션에 그 줄을 추가한다. 이미 지정한 값보다 작게만 줄이도록
"현재 값이 없거나 요청 값보다 큰 파일만" 걸러 적용하는 것이 안전하다.

```bash
# .import 파일 편집이 끝난 뒤, 캐시(.ctex)에 실제로 반영하려면
# 반드시 헤드리스 재임포트를 한 번 돌려야 한다 — .import 텍스트만
# 고치고 재임포트를 건너뛰면 크기가 줄지 않는다.
godot --headless --path . --import
```

Godot 4.6+에서는 여러 텍스처를 선택한 뒤 **Advanced Import Settings for Multiple
Files**로 `Size Limit`을 한 번에 지정하는 에디터 내장 기능도 있다(§5 "여러 파일의
임포트 설정을 한 번에 바꾸기" 참고).

#### 실무 지침

- **UI 아이콘·클로즈업이 잦은 자산은 예외로 둔다** — 위 절약은 "화면에서 작게
  보이는 것"에 크게 적용된다. 카메라가 항상 가까이 붙는 캐릭터 얼굴, 확대해서
  보는 UI 원본 등은 상한을 더 높게 두거나 상한 대상에서 뺀다.
- **정책은 소스가 아니라 임포트 설정에만 둔다** — 원본을 리사이즈해 덮어쓰면
  나중에 더 큰 화면·고사양 빌드가 필요해졌을 때 화질을 복구할 수 없다.
  `size_limit`은 언제든 값을 올리고 재임포트하면 원본 해상도로 돌아온다.
- **플랫폼별로 다른 임포트 프리셋을 쓸 수도 있다** — 데스크톱·콘솔 빌드는 상한을
  더 높게, 모바일 빌드는 낮게 두고 싶다면 플랫폼별 임포트 프리셋
  (Project Settings → Import Defaults, 또는 빌드별로 `.import`를 스크립트로
  다르게 생성)을 검토한다.
