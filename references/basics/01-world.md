...
삼각형 12개 · 정점 24개 (인덱스 36개)
```

> 🔑 **`MeshInstance3D` 는 "메시를 담는 그릇"이지 형상 자체가 아니다.**
> 그릇만 놓아서는 아무 일도 일어나지 않는다.

> ⚠️ **꼭짓점은 8개인데 정점이 24개인 것이 맞다.** 정점은 위치만이 아니라 **노멀·UV 를
> 함께 담은 속성 묶음**이라, 한 꼭짓점이 세 면에 걸치면 정점도 3개가 된다.
> 36은 정점이 아니라 **인덱스** 수다(삼각형 12 × 3).
> → [mesh-geometry.md](../mesh-geometry.md) — 정점·모서리·삼각형·면이 어떻게 이어지는가
